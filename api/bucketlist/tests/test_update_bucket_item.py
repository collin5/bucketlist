...
class UpdateItemTestCase(BaseTestCase):
    ...
    def test_update_item_successfully(self):
        ...
        response = self.app.put('/bucketlists/1/items/1', data=form, headers={"token": self.token})
    ...
    def test_update_item_successfully_content_2(self):
        initial_item = self.app.get(
            '/bucketlists/1/items/1', headers={"token": self.token})
        ...
        update_response = self.app.put('/bucketlists/1/items/1', data=form, headers={"token": self.token})
    ...
    def test_update_item_empty_form(self):
        response = self.app.put('/bucketlists/1/items/1', headers={
            "token": self.token
        })
        self.assertEqual(response.status_code, 200)
        self.assertTrue(
