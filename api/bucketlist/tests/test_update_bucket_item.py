...
class UpdateItemTestCase(TestCase):
    ...
    def test_update_item_successfully_content(self):
        ...
        response = self.app.put('/bucketlists/1/items/1', data=form)
        self.assertEqual(response.status_code, 200)
        self.assertTrue('item successfully updated' in response.data.decode('utf-8').lower())


    @skip
    def test_update_item_successfully_content_2(self):
        initial_item = self.app.get('/bucketlists/1/items/1', data={"token": self.token})

        self.assertEqual(initial_item.status_code, 200)
        self.assertTrue("my awesome item" in initial_item.data.decode('utf-8').lower())
        form = {
                "title": "New title for item",
                "notes":"now this is fun",
                "token": self.token
        ...
        final_item = self.app.get('/bucketlists/1/items/1', data={"token": self.token})
        self.assertTrue("new title for item" in final_item.data.decode('utf-8').lower())
    ...
    def test_update_item_empty_form(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTrue("nothing to update" in response.data.decode('utf-8').lower())
    ...
    def test_update_item_item_out_of_range(self):
        ...
        self.assertEqual(response.status_code, 404)
        self.assertTrue("bucketitem not found" in response.data.decode('utf-8').lower())
